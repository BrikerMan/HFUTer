...
```
pod 'Observable-Swift', :git => 'https://github.com/BrikerMan/Observable-Swift.git'
```

```
github "BrikerMan/Observable-Swift" "HEAD"
```
...
ramsay.last.afterChange += { print("Ramsay \($0) is now Ramsay \($1)") }        
ramsay.last <- "Bolton"
```
...
ramsay += { print("\($0.full) is now \($1.full)") }
...
```swift
var x = Observable(0)    
let subscr = x.afterChange += { (_,_) in print("changed") }
// ...
...
for _ in 0..1 {
    ...
    x.afterChange.add(owner: o) { (oV, nV) in print("\(oV) -> \(nV)") }
    x <- 42 // handler called
