...
public extension StoryboardSceneBased where Self: UIViewController {
  ...
  static func instantiate() -> Self {
    guard let vc = scenceStoryboard.instantiateViewController(withIdentifier: self.sceneIdentifier) as? Self else {
      fatalError("The viewController '\(self.sceneIdentifier)' of '\(scenceStoryboard)' is not of class '\(self)'")
    }
    return vc
  }
}
